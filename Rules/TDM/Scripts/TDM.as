	...
	if (isServer() && ticks_to_water == 0) 
    {
        for (int i = 0; i < getPlayersCount(); i++) 
        {
			...
			|| blob.getName() == "roguecat")
            {
                ...
                blob.server_PutInInventory(bomb); // put the bomb in blob's inventory
            }
        }
    }
}
void Config(TDMCore@ this)
{
	...
	this.kills_to_win_per_player = cfg.read_s32("killsPerTeam", 0);
...
shared class TDMCore : RulesCore
{
	...
	{
		TDMPlayerInfo p(player.getUsername(), player.getTeamNum(), player.getTeamNum() == 0 ? "knight" : "builder");
		players.push_back(p);
