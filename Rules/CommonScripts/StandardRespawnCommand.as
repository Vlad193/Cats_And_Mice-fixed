...
void BuildRespawnMenuFor(CBlob@ this, CBlob @caller)
{
	...
	{
		CGridMenu@ menu = CreateGridMenu(caller.getScreenPos() + Vec2f(24.0f, caller.getRadius() * 1.0f + 48.0f), this, Vec2f((classes.length/2) * CLASS_BUTTON_SIZE, CLASS_BUTTON_SIZE), getTranslatedString("Swap class"));
		if (menu !is null && caller.getTeamNum() == 0)
		{
			addClassesToMenuCats(this, menu, caller.getNetworkID());
		}
		if (menu !is null && caller.getTeamNum() == 1)
		{
			addClassesToMenuMice(this, menu, caller.getNetworkID());
		}
	}
}
