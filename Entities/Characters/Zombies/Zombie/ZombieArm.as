...
void onTick(CBlob@ this)
{
	...
	{	
		...
		for (uint step = 0; step < blobs.length; ++step)
		{
			...
			{
				Vec2f vel(0,0);
				if (other.getTeamNum() != 1) this.server_Hit(other,other.getPosition(),vel,0.2,Hitters::saw, false);
				break;
			}
		}	
	}
		...
		if (lastbite > bitefreq)
		{
			...
			if(this.get_u8(state_property) == MODE_TARGET )
			{
				...
				else vel = Vec2f(1,0);
				{
					...
					if (map.getHitInfosFromArc( this.getPosition()- Vec2f(2,0).RotateBy(-vel.Angle()), -vel.Angle(), 90, this.getRadius() + 4.0f, this, @hitInfos ))
					{
						...
						for (uint i = 0; i < hitInfos.length; i++)
						{
							...
							if (other !is null)
							{
								if (other.hasTag("flesh") && other.getTeamNum() != this.getTeamNum())
								{
									...
									if (other.getTeamNum() != 1) this.server_Hit(other,hi.hitpos,vel,power,Hitters::bite, false);
...
{		
    ...
    if (this.getHealth() <= gibHealth)
    {
		...
		//server_DropCoins(hitterBlob.getPosition() + Vec2f(0,-3.0f), 2);
