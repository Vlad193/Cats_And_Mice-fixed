...
void onRender(CSprite@ this)
{
    ...
    //heal icon
	u32 lastHeal = blob.get_u32("last heal");
	int diff = gametime - (lastHeal + HEAL_FREQUENCY);
	double cooldownHealSecs = (diff / 30) * (-1);
	int cooldownHealFullSecs = diff % 30;
	double cooldownHealSecsHUD;
	if (cooldownHealFullSecs == 0 && cooldownHealSecs >= 0) cooldownHealSecsHUD = cooldownHealSecs;
	
	if (diff > 0)
	{
		GUI::DrawIcon( "healling.png", 0, Vec2f(16,16), Vec2f(11,158));
	...
	else
	{
		...
		GUI::SetFont("menu"); GUI::DrawText("" + cooldownHealSecs + "  R button", Vec2f(25,175), SColor(255, 255, 216, 0));
	}
}
